addSbtPlugin("com.eed3si9n"     % "sbt-assembly"        % "0.14.7")    // cross-platform standalone
addSbtPlugin("com.eed3si9n"     % "sbt-buildinfo"       % "0.9.0")     // meta-data such as project and Scala version
addSbtPlugin("com.typesafe"     % "sbt-mima-plugin"     % "0.3.0")     // binary compatibility testing
addSbtPlugin("com.typesafe.sbt" % "sbt-native-packager" % "1.3.6")     // release standalone binaries
